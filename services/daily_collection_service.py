...
class DailyCollectionService:
    ...
    def create_collection(db: Session, shop_id: int, order_booker_id: int,
                         ...
                         remarks: str = None, visit_id: int = None, auto_commit: bool = True) -> Dict:
        """
        ...
        3. Creates collection with status="verified" (auto-approved, no distributor verification needed)
        ...
        Collections created by order bookers are automatically approved (status="verified").
        ...
        # Create collection with auto-approved status (order booker collections are automatically approved)
        collection = DailyCollectionRepository.create(
            ...
            visit_id=visit_id,
            status="verified",  # Auto-approve collections created by order bookers
            ...
            if collection.collected_by_order_booker:
                WalletService.credit_wallet(
                    ...
                    transaction_metadata={
                        ...
                        "status": collection.status  # Collection status (auto-approved as "verified")
                    }
                )
    ...
    def create_collection_for_delivery_man(db: Session, shop_id: int, delivery_man_id: int,
        ...
        Note: Outstanding balance is reduced immediately so shop can order right away.
        Collections created by delivery men are automatically approved (status="verified").
        ...
        # Create collection with auto-approved status (delivery man collections are automatically approved)
        collection = DailyCollectionRepository.create(
            db=db,
            shop_id=shop_id,
            ...
            collection_date=collected_at,  # Use collection_date to match database
            order_id=order_id,
            status="verified"  # Auto-approve collections created by delivery men
        )
